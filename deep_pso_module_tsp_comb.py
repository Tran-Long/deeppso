...
class DeepPSOModule(L.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.net = TSPVectorGraphCombined(n_particles=n_particles, embed_dim=64)
    ...
    def training_step(self, problem: TSPProblem, idx):
        ...
        wc1c2 = self.net(pyg_data.to(self.device), particle_population.population.to(self.device))
        # Initial evaluation
    ...
    def validation_step(self, problem: TSPProblem, idx, dataloader_idx=0):
        ...
        wc1c2 = self.net(problem.pyg_data.to(self.device), particle_population.population.to(self.device))
        ...
        for iter in range(self.pso_iterations_infer):
            ...
            costs = problem.evaluate(solutions)
            particle_population.update_metadata(costs)
            wc1c2 = self.net(problem.pyg_data.to(self.device), particle_population.population)
