...
class CVRPBatchProblem(BaseProblem):
    ...
    def set_coordinates(
        ...
        if coordinates is not None and demands is not None:
            assert (
                coordinates.shape[:2] == demands.shape[:2]
            ), "Coordinates and demands should have the same batch size and number of cities."
            ...
            if depot_included:
                ...
                assert torch.all(
                    self.demands[:, 0] == 0
                ), "The first demand should be zero for the depot."
            else:
                self.batch_size, self.n_cities, self.n_dims = self.coordinates.shape
                depot = (
                    torch.tensor([self.depot_coor], device=self.device)
                    .unsqueeze(0)
            ...
            self.coordinates = torch.rand(
                size=(self.batch_size, self.n_cities, self.n_dims), device=self.device
            )
            self.demands = torch.randint(
                low=self.demand_low,
                high=self.demand_high + 1,
                size=(self.batch_size, self.n_cities),
                device=self.device,
            )
            depot = (
                torch.tensor([self.depot_coor], device=self.device)
            ...
            self.coordinates = torch.cat((depot, self.coordinates), dim=1)
            self.demands = torch.cat(
                (torch.zeros((self.batch_size, 1), device=self.device), self.demands),
                dim=1,
            )
            self.n_cities += 1  # add depot
        else:
            raise ValueError(
                "Both coordinates and demands should be provided together or both should be None."
            )

        self.distance_matrix = torch.norm(
            self.coordinates[:, :, None] - self.coordinates[:, None, :], dim=3, p=2
        )
        self.distance_matrix[
            torch.arange(self.batch_size, device=self.device)[:, None],
            torch.arange(self.n_cities, device=self.device),
            torch.arange(self.n_cities, device=self.device),
        ] = 1e-10
        ...
        src = (
            torch.arange(self.n_cities, device=self.device)
            .repeat(self.n_cities)
            .repeat(self.batch_size)
        )  # (batch_size*n_cities*n_cities,)
        dst = torch.repeat_interleave(
            torch.arange(self.n_cities, device=self.device), self.n_cities
        ).repeat(
            self.batch_size
        )  # (batch_size*n_cities*n_cities,)

        batch_offsets = (
            torch.arange(self.batch_size, device=self.device) * self.n_cities
        )
        src = src + batch_offsets.repeat_interleave(
            self.n_cities * self.n_cities
        )  # (batch_size*n_cities*n_cities,)
        dst = dst + batch_offsets.repeat_interleave(
            self.n_cities * self.n_cities
        )  # (batch_size*n_cities*n_cities,)

        edge_index = torch.stack((src, dst))  # (2, batch_size*n_cities*n_cities)

        # Enhanced node features: [x_coord, y_coord, demand, normalized_demand, is_depot]
        batch_size, n_cities = self.batch_size, self.n_cities
        coords_flat = self.coordinates.view(
            batch_size * n_cities, 2
        )  # (batch_size*n_cities, 2)
        normalized_demands = (self.demands / self.capacity).view(
            batch_size * n_cities, 1
        )  # (batch_size*n_cities, 1)

        # Create is_depot flag: depot is the first node in each batch
        is_depot = torch.zeros((batch_size * n_cities, 1), device=self.device)
        is_depot[torch.arange(0, batch_size * n_cities, n_cities)] = (
            1.0  # Mark depot nodes
        )

        x_batched = torch.cat(
            [coords_flat, normalized_demands, is_depot], dim=1
        )  # (batch_size*n_cities, 5)
        edge_attr = self.distance_matrix.reshape(
            -1, 1
        )  # (batch_size*n_cities*n_cities, 1)
        batch_alignment = torch.arange(
    ...
    def evaluate(self, solutions):
        ...
        batch_indices = (
            ...
            .unsqueeze(1)
            .unsqueeze(2)
            .expand(-1, n_particles, -1)
        )
        costs = torch.sum(
            self.distance_matrix[batch_indices, u, v], dim=2
        ...
        **kwargs,
        ...
        if n_dims == 2:
            ...
            for val_file in val_files:
                ...
                for i in range(0, dataset.size(0), batch_size):
                    ...
                    problem_instance.set_coordinates(
                        coordinates=coordinates, demands=demands, depot_included=True
                    )
        ...
        for n_ct in all_n_cities:
            if f"{n_ct}_file" not in val_datasets_dict:
                print(
                    f"⚠️ [Warning]: No validation files found for {n_ct}. Generating random instances for these."
                )
        ...
        **kwargs,
        ...
        for n_ct in all_n_cities:
            if f"{n_ct}_file" not in test_datasets_dict:
                print(
                    f"⚠️ [Warning]: No test files found for {n_ct}. Generating random instances for these."
                )
                need_random.append(n_ct)
            elif random_include:
                need_random.append(n_ct)
