...
class PPO(BaseRLAlgorithm):
    def __init__(self, 
        ...
        assert getattr(self.agent, "critic", None) is not None, "PPO requires the agent to have a critic for value estimation."
        assert self.agent.get_action_and_value is not None, "PPO agent must implement get_action_and_value method that returns both action and value estimates."
        assert self.agent.get_value is not None, "PPO agent must implement get_value method that returns value estimates."
    ...
    def training_step(self, env: BaseEnvPSOBatchProblem, idx):
        ...
        with torch.no_grad():
            terminate_values = self.agent.get_value((*observations, None)).squeeze(-1)  # (B,)
        if self.use_gae:
        ...
        flat_obs_problems = reduce(lambda x, y: x + y, [problem_cls.unbatch_instances(prob) for prob in all_obs_problems])    # List of length T*B
        ...
        for _ in range(self.num_updates):
            ...
            for start in range(0, T * B, self.mini_batch_size):
                ...
                mb_obs_problems = problem_cls.batch_instances(*[flat_obs_problems[i] for i in mb_indices])
                ...
                (new_wc1c2, new_log_probs, new_entropy), new_value = self.agent.get_action_and_value(
                    (mb_obs_pop, mb_obs_vel, mb_obs_pbest, mb_obs_gbest, mb_obs_problems.to(self.device), None)
                )
                ...
                value_loss_unclipped = ((new_value.squeeze(-1) - mb_returns) ** 2).mean()
                if self.clip_vloss:
                    value_clipped = mb_values + torch.clamp(new_value.squeeze(-1) - mb_values, -self.clip_coef, self.clip_coef)
