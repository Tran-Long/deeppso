...
class VPG(BaseRLAlgorithm):
    def __init__(
        ...
        **kwargs,
        ...
        ), "VPG requires the agent to have a critic for value estimation."
        assert (
            self.agent.get_action_and_value is not None
        ), "VPG agent must implement get_action_and_value method that returns both action and value estimates."
        assert (
            self.agent.get_value is not None
        ), "VPG agent must implement get_value method that returns value estimates."
        self.gamma = gamma
    ...
    def training_step(self, env: BaseEnv, idx):
        assert (
            env.auto_reset
        ), "Vectorized environments must have auto-reset enabled for VPG training."
        ...
        all_dones = torch.zeros(
            (T, B, P), dtype=torch.bool, device=self.device
        )  # (T, B, P)
        ...
        for t in range(T):
            ...
            observations, rewards, done, _, _ = env.step_train(
                wc1c2=wc1c2, using_random=self.pso_using_random
            )
            done = done.unsqueeze(-1).expand_as(rewards)  # (B, P)
            assert rewards.shape == (
                B,
                P,
            ), f"Expected rewards shape to be (B, P) but got {rewards.shape}"
        ...
        loss = (
            policy_loss + self.vf_coef * value_loss + self.entropy_coef * entropy_loss
        )
